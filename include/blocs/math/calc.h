...
namespace blocs
{
    namespace calc
    {
        ...
        {
            return n < target ? std::min(n + delta, target) : std::max(n - delta, target);
        }

        template <typename T>
		inline T map(T n, T old_min, T old_max, T new_min, T new_max)
        {
            return new_min + ((n - old_min) / (old_max - old_min)) * (new_max - new_min);
        }
        ...
        {
            return map(calc::clamp(n, old_min, old_max), old_min, old_max, new_min, new_max);
        }
        ...
        template<typename T>
        inline vec2<T> lerp_bezier(const vec2<T>& a, const vec2<T>& b, const vec2<T>& c, f32 t)
        {
            return lerp(lerp(a, b, t), lerp(b, c, t), t);
        }

        template<typename T>
        inline vec2<T> lerp_bezier(const vec2<T>& a, const vec2<T>& b, const vec2<T>& c, const vec2<T>& d, f32 t)
        {
        ...
        inline i32 hex(const color& c)
        {
            return hex(c.x * 255, c.y * 255, c.z * 255);
        }

        inline color transparentize(color col, f32 a)
        {
        ...
        inline T smooth_damp(T a, T b, T& curr_val, f32 t, f32 dt)
        {
            ...
            T x = omega * dt;
            T exp = (T)1.0 / ((T)1.0 + x + (T)0.48 * x * x + (T)0.235 * x * x * x);
            T change = a - b;
            T original_to = b;
        ...
        inline vec2<T> smooth_damp(vec2<T> a, vec2<T> b, f32 t, f32 dt)
        {
            ...
            f32 dot = omega * dt;
            f32 exp = (T)1.0 / ((T)1.0 + dot + (T)0.48 * dot * dot + (T)0.235 * dot * dot * dot);
            ...
            if (orig_minus_current_x* out_minus_orig_x + orig_minus_current_y * out_minus_orig_y > (T)0.0)
            {
                output_x = a.x;
                output_y = a.y;
            }
        ...
        template <typename T>
        inline T triple_dot(const vec2<T> vec0, const vec2<T> vec1, const vec2<T> vec2)
		{
			return {
        ...
        {
            return acos(calc::dot(vec0, vec1) / vec0.magnitude() * vec1.magnitude());
        }
        ...
        inline vec2<T> rotate_around(vec2<T> vec, vec2<T> center, const T angle)
        {
            ...
            const vec2<T> rotated = { ca * relative.x - sa * relative.y, sa * relative.x + ca * relative.y };
            return rotated + center;
        }

        template<typename T>
        constexpr inline vec2<T> transform(const vec2<T>& vec, const mat4x4<T>& mat)
        {
            return vec2<T>(
        ...
        template<typename T>
        vec2<T> inline world_to_screen(const vec2<T>& pos, const mat4x4<T>& cam, f32 zoom)
		{
