...
namespace blocs
{
    namespace platform
    {
        namespace filesystem
        {
            ...
            inline str get_file_name(const str& path, bool with_extension = false)
            {
                auto file = static_cast<std::filesystem::path>(path).filename();
                return with_extension ? file.string() : file.replace_extension().string();
            }

            inline str get_file_extension(const str& path)
            {
                return static_cast<std::filesystem::path>(path).extension().string();
            }
            ...
            inline void all_files_in_dir(std::vector<str>& list, cstr path, bool recursive = true)
            {
            ...
            inline void all_files_in_dir(std::vector<str>& list, const str& path, bool recursive = true)
            {
