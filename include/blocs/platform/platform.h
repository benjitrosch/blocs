...
namespace blocs
{
    ...
    namespace platform
    {
         ...
         */
        inline Window init(cstr title, i32 w, i32 h, bool fullscreen, bool vsync, u32 flags)
        {
            ...
            if (window == nullptr)
            {
				LOG_ERR("SDL_CreateWindow HAS FAILED. REASON: " << SDL_GetError());
                return nullptr;
            }
        ...
        inline Renderer create_context(Window window, bool vsync = true)
        {
            ...
            if (context == nullptr)
            {
				LOG_ERR("SDL_GL_CreateContext HAS FAILED. REASON: " << SDL_GetError());
                return nullptr; 
            }
        ...
        inline void set_fullscreen(Window window, bool enabled, bool hardware = false, recti bounds = { 0, 0, 0, 0 })
        {
#ifdef __EMSCRIPTEN__
        ...
        {
            set_fullscreen(window, !(SDL_GetWindowFlags(window) & 
#ifndef __EMSCRIPTEN__
				...
				switch (event.type)
				{
                    ...
                    case SDL_CONTROLLERDEVICEADDED:
                    {
                        ...
                        if (index >= 0 && index < MAX_NUM_CONTROLLERS)
                        {
                            ...
                            u16 version              = SDL_GameControllerGetProductVersion(ctrl);
                            ...
                            input->connect_controller(index, name, vendor, product, version, num_buttons, num_axis);
                    ...
                    {
                        auto index = input->get_controller_index_by_instance_id(event.cdevice.which);
                        if (index >= 0)
                        {
        ...
        inline str get_app_path_str()
        {
            return str(get_app_path());
        }
        ...
        inline void* get_Gl_function()
        {
            return (void*)&SDL_GL_GetProcAddress;
        }

        inline void swap(Window window)
        {
