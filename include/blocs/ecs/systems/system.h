...
namespace blocs
{
    ...
    namespace ecs
    {
        ...
        using Query             = typename std::common_type<std::function<void(T&, Types&...)>>::type;
        template<typename T, typename... Types>
        using QueryWithEntity   = typename std::common_type<std::function<void(Entity, T&, Types&...)>>::type;

        using SetupSystem       = std::function<void(World& world)>;
        using System            = std::function<void(World& world, Time time)>;
