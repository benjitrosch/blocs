...
namespace blocs
{
	namespace ecs
	{
		...
		public:
			/**
			 * @tparam T type of resource to add (automatically inferred from resource parameter).
			...
			T* add(T* resource)
			{
				...
				assert(m_resources.find(type) == m_resources.end() && "ERROR: already registered resource");
			 ...
			 * @tparam Args types of resource arguments (inferred from members of T).
			 * 
			...
			{
				u8 type = get_type_id<T>();
				assert(m_resources.find(type) != m_resources.end() && "ERROR: could not find requested resource");
			...
			*/
			...
			/**
			 * @brief Resets all registered resources to their default values. Does not
			 * remove or unregister resources.
			 */
			void reset()
			{
