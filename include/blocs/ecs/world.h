...
{	
	namespace ecs
	{
		 ...
		 * and provides duplicate API's for common manager methods to allow quick access.
		 */
		struct World
		{
			...
			/**
			 * @brief Creates a new entity from the available pool of id's and adds to game.
			 * Will become available in System component queries next frame.
			 *
			...
			/**
			 * @brief Returns number of active components for the queried component.
			 * 
			...
			size query_count(std::function<bool(T&)> predicate)
			{
				size n = 0;
				auto* component_array = components.get_components<T>();
				for (auto e = component_array->set.end(); e-- != component_array->set.begin();)
				{
					auto entity = *e;
			...
			/**
			 * @brief Runs a lambda expression on every entity with components specified
			 * in the type parameters. The first type parameter specifies the "primary"
			 * component being queried whose component array is iterated over.
			 * 
			 * @tparam Types the components to query for and use as params in the lambda.
			...
			constexpr void query(QueryWithEntity<T, Types...> func)
			{
				...
				for (auto e = component_array->set.end(); e-- != component_array->set.begin();)
				{
					...
					if (size == 0 || components.has<Types...>(entity))
					{
						func(entity, component, components.get<Types>(entity)...);
					}
				}
			}
			...
			{
				ComponentArray<T>* componentArray = components.get_components<T>();
			...
			struct QueryResult
			{
				...
				struct Iterator
				{
					...
					item operator*()
					{
						...
						return { entity, component, world->components.get<Types>(entity)... };
					}
					...
					Iterator& operator++()
					{
						...
						{
							auto entity = *(component_array->set.begin() + index);	
							while (index < component_array->set.len && !world->components.has<Types...>(entity))
							{
								++index;
							}
						}
					...
					Iterator& operator--()
					{
						...
						{
							auto entity = *(component_array->set.begin() + index);	
							while (index > 0 && !world->components.has<Types...>(entity))
							{
								--index;
							}
						}
				...
				Iterator end() const
				{
					return { world, world->components.get_components<T>(), (u16)world->components.get_components<T>()->set.len };
				}
				...
				{
					auto entity = *(world->components.get_components<T>()->set.begin() + index);
					auto& component = world->components.get_components<T>()->get(entity); 
			...
			void start()
			{
				for (auto system : systems.setup_systems)
					system(*this);
			}
			...
			{
				for (auto system : systems.event_systems)
					system(*this, event);
			}
			...
			{
				for (auto system : systems.systems[Stage::DRAW])
					system(*this, time);

				for (auto system : systems.systems[Stage::UI])
					system(*this, time);
			}
		};
	}
