		...
		private:
			...
			*/
			...
			void register_component()
			{
				...
				assert(m_componentArrays[type] == nullptr && "ERROR: already registered component");
				m_componentArrays[type] = new ComponentArray<T>();
			}

		public:
			/**
			 * @brief Creates a new component of type T (with passed in arguments or
			 * default constructor) and associates with an entity.
			 * 
			 * @tparam T type of component to add.
			 * @param entity Entity the component will be added to.
			 * @param args Arguments to pass to the constructor of component type T.
			 * 
			...
			/**
			 * @brief Inserts a premade component of type T into a component array
			 * and associates with an entity.
			 * 
			...
			/**
			 * @brief Calls `reset` on all Component Arrays which reinitializes all
