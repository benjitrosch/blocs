...
namespace blocs
{
	namespace ecs
	{
		...
		struct EntityManager
		{
			...
			{
				assert(entities.len + 1 < ecs::MAX_ENTITIES && "ERROR: max entity limit reached");
				...
				assert(name_to_entity.find(name) == name_to_entity.end() && "ERROR: cannot have duplicate entity names");
			 ...
			 */
			Entity get_by_name(const str& name)
			{
			...
			/**
			 * @brief Finds and removes an from entity among the active entities vector
			 * and returns the id to the availabe entities queue.
			 * 
			 * @param entity Entity to be removed.
			 */
			void remove(Entity entity)
			{
				assert(entity < ecs::MAX_ENTITIES && "ERROR: entity out of range");
				...
				// Return the entity id to back of available queue to be reused later.
				available_entities.push(entity);
				num_active_entities--;
			}
