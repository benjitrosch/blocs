...
That sounds more complex than components and entities but they're just functions. Define a global function with the parameters `(World& world, Time time)` and call `SystemManager::add()`. 

```cpp
void system(World& world, Time time)
{
...
```cpp
void log_high_score(World& world, Time time)
{
