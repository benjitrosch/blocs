...
namespace blocs
{
    namespace network
    {
        namespace
        {
            ...
            inline str concat_headers(const std::vector<std::pair<str, str>> headers)
            {
                str output = "";
        ...
        inline i32 request(const i32 port, const str& url, const str& method = "GET", const str& payload = "")
        {
            ...
            str headers = (create_request_line(url, method) + concat_headers({
        ...
        inline i32 on(const i32 port, const str& url = "/", std::function<str()> func = [](){ return ""; })
        {
#ifdef _WIN32
            auto wsa_data = WSADATA();
            if (WSAStartup(MAKEWORD(2,2), &wsa_data) != 0)
            ...
            if(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, (char*)&true_flag, sizeof(int)) < 0)
#else
            const i32 true_flag = 1;
            if(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &true_flag, sizeof(int)) < 0)
#endif
            {
                log_socket_errors();
                return 1;
            ...
            while (true)
            {
                ...
                if (FD_ISSET(socket_fd, &set))
                {
                    ...
                    if ((client_socket_fd = accept(socket_fd, (struct sockaddr*)&client_addr, &addr_size)) < 0)
                    {
                        log_socket_errors();
                        return 1;
                    }
                    ...
                    char buffer[4096];
                    i32 request;
                    if ((request = recv(client_socket_fd, buffer, sizeof(buffer), 0) < 0))
                    {
                        log_socket_errors();
                    }
                    ...
                    str headers = (create_response_line(has_response ? 200 : 204) + concat_headers({
                        { "Connection", "close" },
                        { "Content-Type", "application/json" },
