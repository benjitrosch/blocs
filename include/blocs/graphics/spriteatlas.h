...
namespace blocs
{
    ...
    public:
        ...
        Node* add(const str& filepath)
        {
            ...
            assert(img.w < MAX_ATLAS_SIZE && img.h < MAX_ATLAS_SIZE && "ERROR: image too large for atlas");
        ...
        Node* add(const u8* pixels, const i32 w, const i32 h)
        {
            ...
            assert(w < MAX_ATLAS_SIZE && h < MAX_ATLAS_SIZE && "ERROR: pixel data too large for atlas");
        ...
        Node* add(const color& col, const i32 w, const i32 h)
        {
            ...
            {
                pixels[i + 0] = (uchar)(col.x * 255U);
                pixels[i + 1] = (uchar)(col.y * 255U);
                pixels[i + 2] = (uchar)(col.z * 255U);
        ...
        {
            assert(w < MAX_ATLAS_SIZE && h < MAX_ATLAS_SIZE && "ERROR: too large for atlas");
        ...
        void pack()
        {
            ...
            assert(max_w < MAX_ATLAS_SIZE && max_h < MAX_ATLAS_SIZE && "ERROR: atlas wider than maximum size");
            assert(area <= MAX_ATLAS_SIZE * MAX_ATLAS_SIZE * SUBOPTIMAL_COEFFICIENT && "ERROR: total area needed cannot fit in maximum size");

            i32 start_w = std::max((i32)std::ceil(std::sqrt(area / SUBOPTIMAL_COEFFICIENT)), max_w);
...
#ifdef DEBUG
            m_timer.stop();
            LOG_INFO("Packed " << nodes.size() << " textures in " << ((f32)m_timer.get_time_elapsed_ms()) << "ms");
#endif
        }
        ...
        void generate_texture()
        {
            ...
            if (!packed)
            {
                LOG_ERR("cannot generate texture before sprite atlas has been packed");
                return;
            }
            ...
            for (auto& node : nodes)
            {
                if (m_expand > 0)
                {
                    ...
                    for (i32 y = -m_expand * 4; y < (node->h + m_expand) * 4; y += 4)
                    {
                        for (i32 x = -m_expand * 4; x < (node->w + m_expand) * 4; x += 4)
                        {
                            i32 src_x = x < 0 ? 0 : x > (node->w - 1) * 4 ? (node->w - 1) * 4 : x;
                            i32 src_y = y < 0 ? 0 : y > (node->h - 1) * 4 ? (node->h - 1) * 4 : y;

                            m_image->pixels[node->x * 4 + x + (node->y * 4 + y) * m_image->w + 0] = src[src_x + src_y * node->w + 0];
                            m_image->pixels[node->x * 4 + x + (node->y * 4 + y) * m_image->w + 1] = src[src_x + src_y * node->w + 1];
                            m_image->pixels[node->x * 4 + x + (node->y * 4 + y) * m_image->w + 2] = src[src_x + src_y * node->w + 2];
                            m_image->pixels[node->x * 4 + x + (node->y * 4 + y) * m_image->w + 3] = src[src_x + src_y * node->w + 3];
                        }
                    }
                }
                else
                {
                    m_image->set(m_buffer + node->buffer_index, { node->x, node->y, node->w, node->h });
                }
            }
...
#ifdef DEBUG
            m_timer.stop();
            LOG_INFO("Generated texture atlas in " << ((f32)m_timer.get_time_elapsed_ms()) << "ms");
#endif
        }
        ...
        Texture* get_texture()
        {
            ...
            if (!generated)
            {
                LOG_ERR("cannot get texture before sprite atlas has been generated");
                return nullptr;
            }
