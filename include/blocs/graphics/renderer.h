...
#ifdef DEBUG
    #define LOG_GL_DEBUG(msg) do {\
        LOG_DEBUG("(OPENGL) " << msg);\
    } while (0)
    #define LOG_GL_ERR(msg, fn, line) do { std::cerr\
        << RED("(OPENGL) [" << fn << ", " << line << "]: "\
		<< msg) << "\n";\
        abort();\
        } while (0)
#else
    #define LOG_GL_DEBUG(msg) do { } while (0)
    #define LOG_GL_ERR(msg)   do { } while (0)
#endif
...
namespace blocs
{
    namespace renderer
    {
        ...
        struct BufferAttribute
        {
            ...
            BufferAttribute(AttributeType type, const str& name, bool normalized = false)
        ...
        namespace attributes
        {
            inline u32 get_size_of_type(AttributeType type)
            {
                ...
                {   
                    ...
                    case AttributeType::BOOL:      return 4;
            ...
            inline GLenum get_GL_type(AttributeType type)
            {
                ...
                {   
                    ...
                    case AttributeType::BYTE:      return GL_UNSIGNED_BYTE;
                    case AttributeType::BOOL:      return GL_BOOL;
                    case AttributeType::INT:       return GL_INT;
                    case AttributeType::FLOAT:     return GL_FLOAT;
                    case AttributeType::VEC2:      return GL_FLOAT;
                    case AttributeType::VEC3:      return GL_FLOAT;
                    case AttributeType::VEC4:      return GL_FLOAT;
        ...
        namespace vertex_buffer
        {
            ...
            inline gl_id generate(size max_size)
            {
                ...
                GL_CALL(glGenBuffers(1, &id));
                GL_CALL(glBindBuffer(GL_ARRAY_BUFFER, id));
                GL_CALL(glBufferData(GL_ARRAY_BUFFER, max_size, nullptr, GL_DYNAMIC_DRAW));
                return id;
            }
        ...
        namespace vertex_array
        {
            ...
            inline void add_buffer(const gl_id id, const gl_id vb, const buffer_layout& layout, i32 divisor = 0)
            {
                ...
                for (u32 i = 0; i < layout.size(); i++)
                {
                    ...
                    GL_CALL(glVertexAttribPointer(i,
                        attributes::get_type_count(attribute.type),
                        attributes::get_GL_type(attribute.type),
                        attribute.normalized,
                        attributes::get_stride(layout),
                        (const void*)offset)
                    );
        ...
        namespace index_buffer
        {
            ...
            inline gl_id generate(size count)
            {
                ...
                GL_CALL(glGenBuffers(1, &id));
                GL_CALL(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, id));
                GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * sizeof(u32), nullptr, GL_DYNAMIC_DRAW));
                return id;
            }
            ...
            inline void subdata(size size, void* data)
            {
                GL_CALL(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, size * sizeof(u32), data));
            }

            inline void buffer_data(size max_size)
            {
                GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, max_size * sizeof(u32), nullptr, GL_DYNAMIC_DRAW));
            }
        }
        ...
        namespace shaders
        {
            constexpr cstr DEFAULT_2D_VERTEX_SHADER =
                ...
                "   v_color = a_color;\n"
                "   v_fill = a_fill;\n"
                "   gl_Position = u_world * vec4(a_pos.xy, 0.0, 1.0);\n"
            ...
            constexpr cstr DEFAULT_2D_FRAGMENT_SHADER =
                ...
                "void main()\n"
                "{\n"
                "   color = texture(u_texture, v_tex) * v_color + v_fill * v_color;\n"
                "}\n";
            ...
            inline gl_id create_shader(const str& vert, const str& frag)
            {
                ...
                assert(compiled_vert != -1 && "could not compile vertex shader");
                assert(compiled_frag != -1 && "could not compile fragment shader");
            ...
            inline void set_uniform(i32 location, const size count, const i32 vals[])
            {
                GL_CALL(glUniform1iv(location, count, vals));
            }

            inline void set_uniform(i32 location, const size count, const vec2<i32> vals[])
            {
            ...
            inline void set_uniform(i32 location, const size count, const vec3<i32> vals[])
            {
            ...
            inline void set_uniform(i32 location, const size count, const vec4<i32> vals[])
            {
            ...
            inline void set_uniform(i32 location, const size count, const f32 vals[])
            {
                GL_CALL(glUniform1fv(location, count, vals));
            }

            inline void set_uniform(i32 location, const size count, const vec2<f32> vals[])
            {
            ...
            inline void set_uniform(i32 location, const size count, const vec3<f32> vals[])
            {
            ...
            inline void set_uniform(i32 location, const size count, const vec4<f32> vals[])
            {
        ...
        namespace textures
        {
            inline gl_id generate(const str& filepath, i32 &w, i32& h, bool aliased = false)
            {
                gl_id id;
...
#ifdef __EMSCRIPTEN__
                GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, img.pixels));
#else
                GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8_REV, img.pixels));
#endif
            ...
            inline gl_id generate(const void* pixels, const i32 w, const i32 h, bool aliased = false)
            {
                gl_id id;
                ...
                GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels));
                
                return id;
            }

            inline void sub_data(const void* pixels, const i32 x, const i32 y, const i32 w, const i32 h)
            {
                GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, w, h, GL_RGBA, GL_UNSIGNED_BYTE, pixels));
            }
        ...
        namespace frame_buffer
        {
            ...
            inline gl_id generate_render_texture(gl_id id, i32 w, i32 h)
            {
                ...
                GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr));
            ...
            inline void generate_render_textures(gl_id id, const size count, gl_id* buffers, i32 w, i32 h)
            {
                glBindFramebuffer(GL_FRAMEBUFFER, id);
                ...
                for (uint i = 0; i < count; i++)
                {
                    ...
                    glBindTexture(GL_TEXTURE_2D, 0);
                    GL_CALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, tex, 0));
                }
        ...
        inline vec4i get_viewport(i32 game_w, i32 game_h, i32 screen_w, i32 screen_h)
        {
        ...
        {
            if (((src != GL_ZERO && src != GL_ONE) && (src < GL_SRC_COLOR || src > GL_ONE_MINUS_SRC1_ALPHA)) ||
                ((dst != GL_ZERO && dst != GL_ONE) && (dst < GL_SRC_COLOR || dst > GL_ONE_MINUS_SRC1_ALPHA)))
            {
        ...
        inline void blend(BlendEquation eq)
        {
            blend((GLenum)eq);   
        }

        inline void blend(BlendFunction color_src, BlendFunction color_dst, BlendFunction alpha_src, BlendFunction alpha_dst)
        {
            GL_CALL(glBlendFuncSeparate((GLenum)color_src, (GLenum)color_dst, (GLenum)alpha_src, (GLenum)alpha_dst));
        }
        ...
        {
            GL_CALL(glBlendColor(col.x, col.y, col.z, col.w));
        }

        inline void enable_blend()
        {
            GL_CALL(glEnable(GL_BLEND));
        }
        ...
        {
            if (face == Cull::NONE) disable_cull();
            else cull((GLenum)face);
        }
        ...
        {
            GL_CALL(glEnable(GL_SCISSOR_TEST));
            GL_CALL(glScissor((GLint)scissor.x, (GLint)scissor.y, (GLint)scissor.z, (GLint)scissor.w););
        }

        inline const uchar* get_renderer()
        {
            return glGetString(GL_RENDERER);
        } 

        inline const uchar* get_version()
        {
        ...
        {
            GL_CALL(glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(i), GL_UNSIGNED_INT, nullptr));
        }

        inline void draw_instanced(size indices = 6, size instances = 1)
        {
            GL_CALL(glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(indices), GL_UNSIGNED_INT, nullptr, static_cast<GLint>(instances)));
        }
    }
}
