...
namespace blocs
{
    ...
    private:
	    ...
	    {
            i32 length = 0;
            cstr name_ptr = stbtt_GetFontNameString(font, &length,
    ...
    public:
        ...
        Character get_character(i32 glyph, f32 size) const
        {
            ...
            stbtt_GetGlyphBitmapBox(m_info, glyph, size, size, &x0, &y0, &x1, &y1);
            ...
            ch.offset_y = (f32)y0;
            ch.size = size;
            ch.has_glyph = (w > 0 && h > 0 && stbtt_IsGlyphEmpty(m_info, glyph) == 0);
        ...
        bool get_image(const Character& ch, u8* pixels) const
        {
            if (ch.has_glyph)
            {
                stbtt_MakeGlyphBitmap(m_info, pixels, ch.width, ch.height, ch.width, ch.size, ch.size, ch.glyph);
                i32 len = ch.width * ch.height;
                for (i32 a = (len - 1) * 4, b = (len - 1); b >= 0; a -= 4, b -= 1)
                {
                    pixels[a + 0] = pixels[b];
                    pixels[a + 1] = pixels[b];
                    pixels[a + 2] = pixels[b];
