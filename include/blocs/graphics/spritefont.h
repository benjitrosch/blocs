...
namespace blocs
{
    ...
    public:
        ...
        SpriteFont() = delete;
        SpriteFont(const str& filepath, f32 size, SpriteAtlas* atlas = nullptr, bool aliased = true)
        {
            Font font = Font(filepath);
            pack_glyphs(font, size, atlas);
        }

        SpriteFont(const Font& font, f32 size, SpriteAtlas* atlas = nullptr, bool aliased = true)
        {
            pack_glyphs(font, size, atlas);
        }
         ...
         */
        void pack_glyphs(const Font& font, f32 size, SpriteAtlas* atlas = nullptr, bool aliased = true)
        {
            ...
            for (i32 a = 0; a < 96; a++)
            {
                for (i32 b = 0; b < 96; b++)
                {
                    ...
                    auto kerning_value = font.get_kerning(ch_a.glyph, ch_b.glyph, size);
                    if (kerning_value != 0)
