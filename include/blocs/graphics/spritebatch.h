...
namespace blocs
{
    ...
    private:
         ...
         */
        constexpr inline rectf normalize_quad(const rectf& src, const f32 w, const f32 h)
        {
            return rectf(src.x / w, src.y / h, src.z / w, src.w / h);
        }
         ...
         */
        constexpr inline Vertex* push_tri(Vertex* target, f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2, const vec4f& c0, const vec4f& c1, const vec4f& c2)
        {
            target->pos = { x0, y0, 0 };
            target->tex = {};
            target->col = c0;
            target->fil = color::white;
            target++;
         ...
         */
        constexpr inline Vertex* push_quad(Vertex* target, f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2, f32 x3, f32 y3, const vec4f& c0, const vec4f& c1, const vec4f& c2, const vec4f& c3)
        {
            target->pos = { x0, y0, 0 };
            target->tex = {};
            target->col = c0;
         ...
         * @return constexpr Vertex* 
         */
        constexpr inline Vertex* push_quad(Vertex* target, const rectf& src, const rectf& dst, const f32 z, const vec4f& color, const vec4f& fill, const SpriteFlip flip)
        {
            vec2f bl = { src.x,         src.y };
            vec2f br = { src.x + src.z, src.y };
            vec2f tr = { src.x + src.z, src.y + src.w };
            vec2f tl = { src.x,         src.y + src.w };
            f32 temp = 0;
            ...
            // Bottom Right
            target->pos = { dst.x + dst.z, dst.y, z };
            target->tex = br;
            target->col = color;
            target->fil = fill;
            target++;
            ...
            // Top Right
            target->pos = { dst.x + dst.z, dst.y + dst.w, z };
            target->tex = tr;
            target->col = color;
            target->fil = fill;
            target++;
            ...
            // Top Left
            target->pos = { dst.x, dst.y + dst.w, z };
            target->tex = tl;
            target->col = color;
            target->fil = fill;
            target++;
         ...
         * @return constexpr Vertex* 
         */
        constexpr inline Vertex* push_quad(Vertex* target, const rectf& src, const rectf& dst, const f32 z, const vec2f org, const f32 sin, const f32 cos, const vec4f& col, const vec4f& fill, const SpriteFlip flip)
        {
            vec2f bl = { src.x,         src.y };
            vec2f br = { src.x + src.z, src.y };
            vec2f tr = { src.x + src.z, src.y + src.w };
            ...
            // Bottom Right
            target->pos = { x + (dx + w) * cos - dy * sin, y + (dx + w) * sin + dy * cos, z };
            target->tex = br;
            target->col = col;
            target->fil = fill;
            target++;
            ...
            // Top Right
            target->pos = { x + (dx + w) * cos - (dy + h) * sin, y + (dx + w) * sin + (dy + h) * cos, z };
            target->tex = tr;
            target->col = col;
            target->fil = fill;
            target++;
            ...
            // Top Left
            target->pos = { x + dx * cos - (dy + h) * sin, y + dx * sin + (dy + h) * cos, z };
            target->tex = tl;
            target->col = col;
            target->fil = fill;
            target++;
        ...
        {                
            ...
            renderer::vertex_buffer::subdata(m_numVertex * sizeof(Vertex), m_vertices);
        ...
        {
            if (m_numVertex >= MAX_QUADS * VERTEX_PER_QUAD)
                flush();
            ...
            if (m_shouldSetTexture)
                set_texture(sprite.tex);
            ...
            m_buffer = sprite.rot == 0 ?
                push_quad(m_buffer, normalize_quad(sprite.src, sprite.tex_w, sprite.tex_h), sprite.dst, sprite.z, sprite.col, sprite.fil, sprite.flip) :
                push_quad(m_buffer, normalize_quad(sprite.src, sprite.tex_w, sprite.tex_h), sprite.dst, sprite.z, sprite.org, sin(sprite.rot), cos(sprite.rot), sprite.col, sprite.fil, sprite.flip);
        ...
        void draw_shape(ShapeQuad shape)
        {
            ...
            m_buffer = push_quad(m_buffer, shape.x0, shape.y0, shape.x1, shape.y1, shape.x2, shape.y2, shape.x3, shape.y3, shape.c0, shape.c1, shape.c2, shape.c3);
            m_numVertex += VERTEX_PER_QUAD;
        }
        ...
        void draw_shape(ShapeTri shape)
        {
            ...
            m_buffer = push_tri(m_buffer, shape.x0, shape.y0, shape.x1, shape.y1, shape.x2, shape.y2, shape.c0, shape.c1, shape.c2);
         ...
         */
        ShapeQuad get_quad(const vec2f& start, const vec2f& end, f32 t, const color& col0, const color& col1)
        {
            vec2f perp;
    ...
    public:
        ...
        {            
            ...
            m_vbo = renderer::vertex_buffer::generate(MAX_QUADS * VERTEX_PER_QUAD * sizeof(Vertex));
            ...
            m_defaultShader = { renderer::shaders::generate(renderer::shaders::DEFAULT_2D_VERTEX_SHADER,   renderer::shaders::DEFAULT_2D_FRAGMENT_SHADER) };
        }
        ...
        )
        {
            ...
            renderer::blend(blend.color_src, blend.color_dst, blend.alpha_src, blend.alpha_dst);
            ...
            shader.apply("u_flip", renderer::frame_buffer::is_bound() ? -1.f : 1.f);
         ...
         * @param scissor   Masks sprite outside scissor bounds.
         */
        void begin(const mat4x4f& transform, renderer::BlendMode blend, renderer::Cull cull = renderer::Cull::NONE, const rectf& scissor = rectf::one * -1)
        {
         ...
         * @param scissor   Masks sprite outside scissor bounds.
         */
        void begin(const mat4x4f& transform, renderer::Cull cull, const rectf& scissor = rectf::one * -1)
        {
            begin(transform, m_defaultShader, renderer::BlendMode::NonPremultiplied, cull);
        }
        ...
        {
            begin(transform, m_defaultShader, renderer::BlendMode::NonPremultiplied, renderer::Cull::NONE, scissor);
        }
        ...
        void draw(Texture* tex,
            const rectf& src,
            const rectf& dst,
            const vec2f& org = vec2f::zero,
            f32 rot = 0,
            f32 z = 0,
            const color& col = color::white,
            const color& fill = color::clear,
        ...
        )
        {
            ...
            draw_sprite({ src, dst, col, fill, org, rot, z, tex->id, (f32)tex->w, (f32)tex->h, flip });
        }
        ...
        void draw(Texture* tex, vec2f src, vec2f dst, color col = color::white)
        {
            draw(
                tex,
                { src.x, src.y, (f32)tex->w, (f32)tex->h },
                { dst.x, dst.y, (f32)tex->w, (f32)tex->h },
                vec2f::zero,
                0,
                0,
                col
        ...
        )
        {
            ...
            draw_sprite({ src, dst, col, fill, org, rot, z, tex, src.z, src.w, flip });
        }

        /**
         * @brief Draw multiple glyph quads for a string as part of the current batch.
         * 
         ...
         * @param rot   Rotation.
         */
        void text(const SpriteFont& font,
            cstr str,
            f32 x,
            f32 y,
            const vec2f scale = vec2f::one,
            const color& col = color::white,
            f32 rot = 0.0f
        )
        {
            ...
            { 
                ...
                if (ch.node != nullptr)
                {
                    ...
                    if (prev > 0)
                        at.x += font.get_kerning(*str, prev);
                    auto node = ch.node;
                    Texture* tex = node->texture;
                    rectf src = { (f32)node->x, (f32)node->y, (f32)node->w, (f32)node->h };
                    rectf dst = { x + at.x, y + at.y, (f32)node->w * scale.x, (f32)node->h * scale.y };
         ...
         */
        void line(const vec2f& start, const vec2f& end, f32 t, const color& col0, const color& col1)
        {
         ...
         * @param col1  End color.
         */
        void bezier(const vec2f& start, const vec2f& b, const vec2f& end, f32 t, i32 steps, const color& col0, const color& col1)
        {
            f32 add = 1.0f / steps;
         ...
         * @param col   Line color.
         */
        void bezier(const vec2f& start, const vec2f& b, const vec2f& end, f32 t, i32 steps, const color& col)
        {
         ...
         * @param col1  End color.
         */
        void bezier(const vec2f& start, const vec2f& b, const vec2f& c, const vec2f& end, f32 t, i32 steps, const color& col0, const color& col1)
        {
            f32 add = 1.0f / steps;
         ...
         * @param col   Color.
         */
        void bezier(const vec2f& start, const vec2f& b, const vec2f& c, const vec2f& end, f32 t, i32 steps, const color& col)
        {
         ...
         */
        void empty_triangle(const vec2f& p0, const vec2f& p1, const vec2f& p2, const f32 t, const color& col)
        {
            line(p0, p1, t, col);
            line(p1, p2, t, col);
            line(p2, p0, t, col);
         ...
         */
        void triangle(const vec2f& p0, const vec2f& p1, const vec2f& p2, const color& col)
        {
            draw_shape({
                p0.x,
                p0.y,
         ...
         * @param col Outline color.
         */
        void empty_rect(const f32 x, const f32 y, const f32 w, const f32 h, f32 t, const color& col)
        {
         ...
         * @param col Outline color.
         */
        void empty_rect(const vec2f& pos, const f32 w, const f32 h, f32 t, const color& col)
        {
        ...
        {
            empty_rect(r.x, r.y, r.z, r.w, t, col);
        }
         ...
         * @param col Fill color.
         */
        void rect(const f32 x, const f32 y, const f32 w, const f32 h, const color& col)
        {
         ...
         * @param col    Outline color.
         */
        void empty_circle(const vec2f& pos, const f32 radius, f32 t, const i32 steps, const color& col)
        {
            ...
            for (i32 i = 1; i <= steps; i++)
            {
                vec2f at = calc::vec_from_angle<f32>(i * (M_PI / 2) / steps) * radius;
            ...
            ShapeQuad seg0 = { prev_seg3.x1, prev_seg3.y1, first_seg0.x0, first_seg0.y0, prev_seg3.x2, prev_seg3.y2, first_seg0.x3, first_seg0.y3, col, col, col, col };
            ShapeQuad seg1 = { prev_seg2.x1, prev_seg2.y1, first_seg1.x0, first_seg1.y0, prev_seg2.x2, prev_seg2.y2, first_seg1.x3, first_seg1.y3, col, col, col, col };
            ShapeQuad seg2 = { prev_seg0.x1, prev_seg0.y1, first_seg2.x0, first_seg2.y0, prev_seg0.x2, prev_seg0.y2, first_seg2.x3, first_seg2.y3, col, col, col, col };
            ShapeQuad seg3 = { prev_seg1.x1, prev_seg1.y1, first_seg3.x0, first_seg3.y0, prev_seg1.x2, prev_seg1.y2, first_seg3.x3, first_seg3.y3, col, col, col, col };
            draw_shape(seg0);
            draw_shape(seg1);
            draw_shape(seg2);
            draw_shape(seg3);
         ...
         * @param col    Fill color.
         */
        void circle(const vec2f& pos, const f32 radius, const i32 steps, const color& col)
        {
            ...
            for (i32 i = 1; i <= steps; i++)
            {
                ...
                const vec2f next = { pos.x + cos(radians) * radius, pos.y + sin(radians) * radius };
         ...
         * @param col    Outline color.
         */
        void empty_ellipse(const vec2f& center, const i32 maj, const i32 min, const f32 rot, f32 t, const i32 steps, const color& col)
        {
            f32 max = M_PI;
            f32 step = 2 * max / (f32)steps;

            vec2f pos = { center.x + maj * (f32)cos((f64)-max), center.y + min * (f32)sin((f64)-max) };
            ...
            {
                last = pos;
                pos = { center.x + maj * (f32)cos((f64)i), center.y + min * (f32)sin((f64)i) };
                if (rot != 0) pos = calc::transform(pos, mat4x4f::rotateZ(rot));
            ...
            last = pos;
            pos = { center.x + maj * (f32)cos((f64)-max), center.y + min * (f32)sin((f64)-max) };
            if (rot != 0) pos = calc::transform(pos, mat4x4f::rotateZ(rot));
            auto seg = get_quad(last, pos, t, col, col);
            connect_quads(prev_seg, seg);
            draw_shape(seg);
            ...
            seg = { prev_seg.x1, prev_seg.y1, first_seg.x0, first_seg.y0, prev_seg.x2, prev_seg.y2, first_seg.x3, first_seg.y3, col, col, col, col };
            draw_shape(seg);
        }
    };
    ...
    public:
        ...
        {   
            ...
            {
                { { 0.f, 0.f, 0.f }, { 0.f, 0.f }, color::white, color::clear },  //  3-----2
                { { 1.f, 0.f, 0.f }, { 1.f, 0.f }, color::white, color::clear },  //  |   / |
                { { 1.f, 1.f, 0.f }, { 1.f, 1.f }, color::white, color::clear },  //  | /   |
                { { 0.f, 1.f, 0.f }, { 0.f, 1.f }, color::white, color::clear },  //  0-----1
            };

            m_vao = renderer::vertex_array::generate();
            m_vbo = renderer::vertex_buffer::generate(vertices, VERTEX_PER_QUAD * sizeof(Vertex));
            ...
            m_shader = { renderer::shaders::generate(renderer::shaders::DEFAULT_2D_VERTEX_SHADER, renderer::shaders::DEFAULT_2D_FRAGMENT_SHADER) };
        }
        ...
        void draw(
            Shader shader,
            mat4x4f matrix = mat4x4f::ortho(1, 1),
            renderer::BlendMode blend = renderer::BlendMode::NonPremultiplied,
            renderer::Cull cull = renderer::Cull::NONE
        )
        {
        ...
        {
            draw(texture, m_shader, renderer::BlendMode::NonPremultiplied, cull);
        }
    };
}
